...
export default function HowItWorks({ initialData }: { initialData: InitialDataProps }) {
    ...
    // استخراج وضعیت از دیتای ورودی
    const isLoggedIn = !!initialData.user;
    
    // گرفتن اسم واقعی کاربر (اولویت: full_name از پروفایل > name از کاربر)
// این تابع رو پیدا کن و عوض کن:
const getUserFullName = (): string => {
    // اول از user.name استفاده کن (چون داشبورد از این استفاده میکنه و درسته)
    if (initialData.user?.name) {
        return initialData.user.name;
    }
    // اگه نبود از profile.full_name
    if (initialData.profile?.full_name) {
        return initialData.profile.full_name;
    }
    return "";
};
            ...
            <div className="max-w-7xl mx-auto">
                <div className="text-center mb-20 space-y-4">
                    <h2 className="text-3xl md:text-5xl font-black text-stone-800 dark:text-white leading-tight">
                        ...
                        <span className="text-sage-600">{isLoggedIn ? "پیشرفت " : "آموزش "}</span>
